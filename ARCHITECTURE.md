...
- **Strict Blocking Referential & Resource Integrity Validation**:
  - **Comprehensive Referential Integrity Check**: Pre-restore validation dry-run checks referential integrity across all 22 domain entities (StudentEnrollment -> StudentPerson/AcademicYear/Class; Class -> AcademicYear/School; GradingScheme -> AcademicYear; Lesson -> AcademicYear/Class/Sequence/Rubric/Version; Attendance -> Lesson/Enrollment/Class; Assessment -> AcademicYear/Class/GradingScheme/Sequence; GradeEntry -> Assessment/Enrollment; Homework -> Class/AcademicYear/Lesson; Observation -> Enrollment/Person/Class/AcademicYear; Remediation -> AcademicYear/Class/Sequence/Competency/Enrollment; Timetable -> AcademicYear/Class/School; CurriculumLevel/Competency/SessionRubric/CurriculumSequence/LearningObjective -> CurriculumVersion).
  - **Strict Resource Integrity Alignment**: Enforces 1-to-1 matching between `resources` table records and binary payloads (`resources/<id>.bin`). Rejects missing binary files, unexpected orphan binary files in the ZIP archive, and size or SHA-256 checksum mismatches.
  - **Zero-Tolerance Restoration Blocking**: Any referential integrity violation or resource binary mismatch contributes directly to `errors` in `ValidationReport`, setting `isValid = false` and `backupPackage = null`. Restoring invalid archives is strictly forbidden; bad backup packages are never partially repaired or mutated.
